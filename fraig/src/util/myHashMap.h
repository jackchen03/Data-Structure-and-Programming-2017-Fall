...
//
#define PRIME 15077
#define HASH_SIZE 5999


class HashKey{
public:
	HashKey(size_t a, size_t b) : fan_in_0(a), fan_in_1(b) {}
	HashKey(){;	}
   size_t operator() () const {
	  return ( (size_t)(fan_in_0 / PRIME) )*(fan_in_0 % PRIME) + ( (size_t)(fan_in_1 / PRIME) )*(fan_in_1 % PRIME);
   }

   
   bool operator == (const HashKey& k) const { 
      if(fan_in_0 == k.fan_in_0 && fan_in_1 == k.fan_in_1 || fan_in_0 == k.fan_in_1 && fan_in_1 == k.fan_in_0) return true;
      else return false;
   }
...
public:
   ...
   bool check(const HashKey& k) const {
      for(int i=0;i<_buckets[bucketNum(k)].size();i++){
	  	 if(_buckets[bucketNum(k)].at(i).first == k) return true;
	  } 
	  return false;
   }
   ...
   bool query(const HashKey& k, HashData& d) const { 
      for(int i=0;i<_buckets[bucketNum(k)].size();i++){
	  	 if(_buckets[bucketNum(k)].at(i).first == k){
	  	 	d = _buckets[bucketNum(k)].at(i).second;
		    return true;
		 } 
	  }
	  return false;
   ...
   bool update(const HashKey& k, HashData& d) { 
      for(int i=0;i<_buckets[bucketNum(k)].size();i++){
	  	 if(_buckets[bucketNum(k)].at(i).first == k){
	  	 	_buckets[bucketNum(k)].at(i).second = d;
	        return true;
		 } 
	  }
   ...
   bool remove(const HashKey& k) { 
	  for(int i=0;i<_buckets[bucketNum(k)].size();i++){
	  	if(_buckets[bucketNum(k)].at(i).first == k){
		    _buckets[bucketNum(k)].pop_back(); return true;
		}
	  } 
	  return false;
   }
