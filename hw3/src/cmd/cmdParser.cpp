...
{
   // TODO...
   if(_dofileStack.size() > DOFILE_LIMIT ) return false;
...
CmdParser::parseCmd(string& option)
{
   ...
   // TODO...
   string tok;
   int end = myStrGetTok(str, tok);  
   
   if(getCmd(tok)!=NULL){
   	   for(int i=0; end+1+i < str.size() ;i++){    // store in option   	  
		  if(end==-1) break; 
   	      else option.push_back( str.at(end+1+i));
		  if(option.at(i) == 0 ) break;	 	
	   } 
...
CmdParser::listCmd(const string& str)
{
   ...
   bool allemp = 1;  // to see if it's all empty
   ...
   if(allemp == 1) {   // all empty (case 1)
   	  int count = 0; 
   	  cout << endl;
   	  for( CmdMap::const_iterator it = _cmdMap.begin(); it!=_cmdMap.end(); ++it, ++count){
      ...
      reprintCmd();
       ...
       int match = 0;  //number of candidates
	   vector <string> matchcmd;  //vector to store candidates
	   string cmdtok ;
	   int cmdEnd = myStrGetTok(str, cmdtok);
	   CmdExec* match_first;
         ...
         for( CmdMap::const_iterator it = _cmdMap.begin(); it!=_cmdMap.end(); ++it){
            string name;
            name = it -> first; 
            name += it-> second->getOptCmd();   // name = command name
            
            if(name.size() < cmdtok.size()) continue;
            if(myStrNCmp(name, cmdtok, cmdtok.size()) == 0){
            	match ++;
            	matchcmd.push_back(name);      
				match_first = it->second;         
			} 
         } 
		 ...
		 if(cmdEnd != -1){
		 	if(match != 1) mybeep();
		 	else {   //match == 1 && cmdEnd != -1
			   if(_tabPressCount == 1){			     	
			     	cout << endl ;
			     	match_first -> usage(cout) ;
			     	reprintCmd();
			   }
			   else if(_tabPressCount > 1){   // tab pressed more than once
			       vector<string> fileN ;
			   	   ...
			   	   if(str.back() == ' ' ){  // 6.1
			   	   	  ...
			   	   	  listDir(fileN, "",".");
			   	   	  int count = 0;
			   	   	  for(size_t i=0;i<fileN.size();i++){
			   	   	  	  count ++;
				          cout << setw(16) << left << fileN.at(i);
				          if(count %5 == 0) cout << endl;
					  }
					  reprintCmd();
				   } 
				   else{   //6.2  3 4 5
				        string prefix = str.substr(cmdEnd+ 1);				        
				   	   listDir(fileN, prefix, ".");
				   	   if(fileN.size() == 1) {  //6.4
				   	      for(int i=prefix.size();i<fileN.at(0).size();i++){
				   	         insertChar(fileN.at(0).at(i));  	
						  }	
						  insertChar(' ');
					   }
					   else if(fileN.size() == 0 ) mybeep();
					   else if(fileN.size() > 1){
					   	  vector<char> common ; 		   	  
					   	  
					   	  int  minsize = fileN.at(0).size();
					   	  for(int i=1;i<fileN.size();i++){  //determine the min size
					   	  	 if(fileN.at(i).size() < minsize)  minsize = fileN.at(i).size();
						  }
						  
						  for(int i=0;i<minsize;i++){
						  	 bool comm = 1 ;
						  	 char comchar = fileN.at(0).at(i);					  	 
						  	 
						  	 for(int j=1;j<fileN.size();j++) {
						  	 	if(fileN.at(j).at(i) != comchar ){
						  	 	   comm = 0;
								   break;  	
								 }
							 } 
							 if(comm == 1) common.push_back(comchar);
						  }
						  
						  if(common.size() > prefix.size()){
						  	 for(int i=prefix.size();i<common.size();i++) insertChar(common.at(i));
		 ...
		 else if(match == 1 && cmdEnd == -1){		 
			for(int i=cmdtok.size();i<matchcmd.at(0).size();i++) insertChar(matchcmd.at(0).at(i) );
			insertChar(' ' )  ;		
		 } 
         else if(match > 1){
         	 cout << endl;
         	 for(size_t i=0;i<matchcmd.size();i++){
         	 	cout <<  setw(12) << left  << matchcmd.at(i);
         	 	if(i%5 == 4) cout <<endl;
			 } 
         	 reprintCmd();
		 }
...
{  
   ...
   for( CmdMap::const_iterator it = _cmdMap.begin(); it!=_cmdMap.end(); ++it){
   	  string compare; 
   	  compare = it->first ;
	  compare += it->second->getOptCmd();
	  	  
	  if(cmd.size() < it->first.size()) continue;
	  	  
	  if(myStrNCmp( compare, cmd,it->first.size()) == 0 ) {
	  	 e =  it->second;	  	
	  }		
