...
class MemRecycleList
{
   ...
   T* popFront() {
      // TODO
	  T* tmp = _first ;      
      if(getNext(_first)) _first = getNext(_first);
      else _first = NULL ;
      return tmp;
   }
...
public:
   ...
   void reset(size_t b = 0) {
      ...
      // TODO
      	 ...
      	 MemBlock<T>* _currentBlock = new MemBlock<T>(0, _blockSize);
		 _currentBlock = _activeBlock; 
	
		 while(_currentBlock->_nextBlock){
		 	MemBlock<T>* tmp = _currentBlock;
		 	_currentBlock = _currentBlock -> _nextBlock;
	//	 	cerr<<"deleting: "<<tmp<<endl;
		    delete tmp;
		 }		 
		 //reset last block
		 _currentBlock -> reset();
		 _activeBlock = _currentBlock;
		 
		 // resize
		 if(b){ 
		// 	cerr<<"deleting: "<<_currentBlock<<endl;
		    delete _currentBlock;
		    _blockSize = b;
		    _activeBlock = new MemBlock<T>(0, _blockSize);
		//    cerr<<"new block: "<<_activeBlock<<endl;
	  ...
	  for(int i=0;i<R_SIZE;i++){
	  	 ...
	  	 if(_recycleList[i]._nextList == NULL ) {
	  	 	if( _recycleList[i]._first == NULL ){
            ...
            MemRecycleList<T>* _current; 	  
			_current = &_recycleList[i]	; 
            while(_current){
            	MemRecycleList<T>* tmp = _current;
            	_current = _current -> _nextList;
            	tmp->reset();
			}
   ...
   void print() const {
      ...
      cout << "* Number of blocks      : " << getNumBlocks() << endl;
      cout << "* Free mem in last block: " << _activeBlock->getRemainSize()
           << endl
           << "* Recycle list          : " << endl;
      int i = 0, count = 0;
      while (i < R_SIZE) {
...
private:
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      ...
      // TODO
      for(int i=0;;i++){
      	 MemRecycleList<T>* _currentList; 
      	 if(i == 0){
      	     _currentList = &(_recycleList[m]);
			 if(m == n)  return _currentList;	
		 } 
	     else{
	     	if(_currentList->_nextList == NULL ){
	     	   	
	     	   	MemRecycleList<T>* _list = new MemRecycleList<T>;
	     	   	_list->_arrSize = n;
	     	   	_currentList->_nextList = _list;
	     	   	return _list;
			} 
	     	_currentList = _currentList->_nextList ;
	     	if(_currentList->_arrSize == n)  return _currentList;	
   ...
   T* getMem(size_t t) {
      ...
      //    => "ret" is the return address
        ...
        if((getMemRecycleList(arrSize)->_first)) ret = getMemRecycleList(arrSize)->popFront();   
         

      // If no match from recycle list...
      // 4. Get the memory from _activeBlock
        else if((_activeBlock->getMem(t,ret)) == false ){
   ...
   size_t getNumBlocks() const {
      ...
      for(int i=1;;i++){
      	 MemBlock<T>* _current ;
      	 if(i == 1){
      	   _current = _activeBlock;	
      
		 }
		 else{
		 
		 	if((_current->_nextBlock) == NULL)  return i-1 ;	
